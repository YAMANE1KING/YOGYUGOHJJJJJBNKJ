...
if( !route(1) ){
  ...
  $tickets = $conn->prepare("SELECT * FROM payments WHERE client_id=:c_id ");
...
if ($_POST && $_POST["payment_bank"]):
  ...
  $url = '/lib/stripe/index.php';
 
$_SESSION['ODR']= $icid;
$_SESSION['AMT'] = $amount;
$insert = $conn->prepare("INSERT INTO payments SET client_id=:c_id, payment_amount=:amount, payment_privatecode=:code, payment_method=:method, payment_mode=:mode, payment_create_date=:date, payment_ip=:ip, payment_extra=:extra");
                    $insert->execute(array("c_id" => $user['client_id'], "amount" => $amount, "code" => $paymentCode, "method" => $method_id, "mode" => "Automatic", "date" => date("Y.m.d H:i:s"), "ip" => GetIP(), "extra" => $_SESSION['ODR']));

 

 echo '<form action="'.$url.'" name="frm1" method="post">
      <p>Please wait.......</p>
      ...
      <input type="hidden" name="orderAmount"  value="' . $amount . '" />
            ...
            elseif ($method_id == 11):
                ...
                elseif ($method_id == 12):
                    ...
                    $insert->execute(array("c_id" => $user['client_id'], "amount" => $amount/75, "code" => $paymentCode, "method" => $method_id, "mode" => "Automatic", "date" => date("Y.m.d H:i:s"), "ip" => GetIP(), "extra" => $icid));
