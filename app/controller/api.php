...
$update = $conn->prepare("UPDATE panel_info SET panel _status=:status WHERE panel_id=:id");
        ...
        elseif ($action == "status") :
            $order = $conn->prepare("SELECT * FROM orders WHERE order_id=:id AND client_id=:client ");
...
$last_id = $conn->lastInsertId();
        ...
        elseif ($action == "refill_status") :
            ...
            $refill = $conn->prepare("SELECT * FROM refill_status WHERE  refill_apiid=:refill_apiid AND client_id=:client ");
...
elseif ($action == "allorders") :
        ...
        elseif ($action == "services") :
            $servicesRows = $conn->prepare("SELECT * FROM services INNER JOIN categories ON categories.category_id=services.category_id WHERE categories.category_type=:type2 AND services.service_type=:type  ORDER BY categories.category_line,services.service_line ASC ");
        ...
        elseif ($action == "add") :
            ...
            $search2 = $conn->prepare("SELECT * FROM clients_category WHERE category_id=:category AND client_id=:c_id ");
...
$orders = $conn->prepare("SELECT *,services.service_id as service_id,services.service_api as api_id FROM orders
  ...
  WHERE orders.dripfeed=:dripfeed AND orders.subscriptions_type=:subs AND orders.order_status=:statu AND orders.order_error=:error AND orders.order_detail=:detail LIMIT 10 ");
