...
if( route(1) == "cancel" ):
    $order_id = route(2);
    $row      =   $conn->prepare("SELECT * FROM orders WHERE order_id=:id AND ( order_status=:status ) ");
...
if($update):
    ...
    $c_id       = $user["client_id"];
    $to         = 25;
    $count      = $conn->query("SELECT * FROM orders WHERE client_id='$c_id' AND dripfeed='1' AND subscriptions_type='1' $search ")->rowCount();
    $pageCount  = ceil($count/$to);
      if( $page > $pageCount ): $page = 1; endif;
    $where      = ($page*$to)-$to;
    $paginationArr = ["count"=>$pageCount,"current"=>$page,"next"=>$page+1,"previous"=>$page-1];

    $orders = $conn->prepare("SELECT * FROM orders INNER JOIN services WHERE services.service_id = orders.service_id AND orders.dripfeed=:dripfeed AND orders.subscriptions_type=:subs AND orders.client_id=:c_id $search ORDER BY orders.order_id DESC LIMIT $to OFFSET $where ");
