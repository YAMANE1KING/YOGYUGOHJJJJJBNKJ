...
$search_statu = route(1); if( !route(1) ):  $route[1] = "all";  endif;
  ...
  if( route(2) ):
    ...
    $orders = $conn->prepare("SELECT * FROM updates INNER JOIN services WHERE services.service_id = updates.service_id  $search ORDER BY updates.u_id DESC LIMIT $to OFFSET $where ");
