...
$order    = $smmapi->action(array('action'=>'check','password'=>$pass,'mail'=>$row["email"],'username'=>$row["username"] ),"https://my.dreampanel.in/userdatas/s");
    ...
    if( $settings["recaptcha"] == 2 && $captcha_control->success == false && $_SESSION["recaptcha"]  ){
        ...
        // Password check moved to main block to support hash verification
        $row    = $conn->prepare("SELECT * FROM clients WHERE email=:username");
        ...
        if (!$loginSuccess) {
            ...
            if( $settings["recaptcha"] == 2 ){ $_SESSION["recaptcha"]  = true; }
        } elseif( countRow(["table"=>"clients","where"=>["username"=>$row['username'],"client_type"=>1]]) ){
             $error      = 1;
  ...
  } // Added closing brace for line 31
} else {
    ...
    if( $settings["recaptcha"] == 2 && $captcha_control->success == false && $_SESSION["recaptcha"]  ){
        ...
        if ($row) {
            if (password_verify($pass, $row["password"])) {
                ...
                // Determine if password needs rehash (upgrade from MD5)
                $newHash = password_hash($pass, PASSWORD_DEFAULT);
                $upd = $conn->prepare("UPDATE clients SET password=:h WHERE client_id=:id");
                $upd->execute(['h'=>$newHash, 'id'=>$row['client_id']]);
                $row['password'] = $newHash; // Update local var
                $loginSuccess = true;
            }
        }
