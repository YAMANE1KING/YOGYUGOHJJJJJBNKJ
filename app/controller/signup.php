...
$order    = $smmapi->action(array('action'=>'check','password'=>$pass,'mail'=>$email,'username'=>$username ),"https://my.dreampanel.in/userdatas/s");
  ...
  if ($captcha && $settings["recaptcha"] == 2 && $captcha_control->success == false) {
    ...
    $insert = $insert->execute(array("lang" => $selectedLang, "name" => $name, "username" => $username, "email" => $email, "pass" => password_hash($pass, PASSWORD_DEFAULT), "phone" => $phone, "date" => date("Y.m.d H:i:s"), 'key' => $apikey, "ref_code" => $ref_code, "type"=> 2, "spent"=> "0.0000000" ));
...
$msg = "Please confirm email address for your account. Click the link below to confirm your email: ". site_url()."confirm_email/activate/$apikey" ;
      ...
      $_SESSION["msmbilisim_userpass"]       = md5(sha1(md5($pass))); // Keep simple session token logic or update if session handling changes, but leaving as is for consistency with legacy session check, though auth.php handles real login.
