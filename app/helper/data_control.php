...
function getRows($data){
    ...
    if( $data["limit"] ): 
        if (strpos($data["limit"], ',') !== false) {
             list($offset, $count) = explode(',', $data["limit"]);
...
function dayCharge($day,$ay,$year,$extra=null){
    ...
    if( count($extra["status"]) ):
        $where  = "AND ( ";
        if( in_array("cron",$extra["status"]) ): $where .= "order_detail='cronpending' OR "; endif;
        if( in_array("fail",$extra["status"]) ): $where .= "order_error!='-' OR "; endif;
    ...
    $first  = $year."-".$ay."-".$day." 00:00:00";
    $last   = $year."-".$ay."-".$day." 23:59:59";
    $row    = $conn->query("SELECT SUM(order_charge) FROM orders WHERE order_create<='$last' AND order_create>='$first' AND dripfeed='1' AND subscriptions_type='1'   $where   ")->fetch(PDO::FETCH_ASSOC);
...
function monthChargeNet($month,$year,$extra=null){
    ...
    $first  = $year."-".$month."-1 00:00:00";
    $last   = $year."-".$month."-31 23:59:59";
    $row    = $conn->query("SELECT SUM(order_profit) FROM orders WHERE order_create<='$last' AND order_create>='$first' AND dripfeed='1' AND subscriptions_type='1' AND order_api!='0' $where  ")->fetch(PDO::FETCH_ASSOC);
    $row2   = $conn->query("SELECT SUM(order_charge) FROM orders WHERE order_create<='$last' AND order_create>='$first' AND dripfeed='1' AND subscriptions_type='1'  $where  ")->fetch(PDO::FETCH_ASSOC);
